...
# 2. Custom CSS for Amazon Cards & UI
st.markdown("""
    <style>
...
# 4. DATA MAPPING (Old to Ultra-New 2026)
cpu_display_map = {
    "Intel Core i9": "Intel Core i9-14980HX / Ultra 9 (Extreme Gaming/Workstation)",
    "Intel Core i7": "Intel Core i7-13700H / Ultra 7 (Video Editing/Pro)",
    "Intel Core i5": "Intel Core i5-13500H / Ultra 5 (Multitasking/Office)",
    "AMD Ryzen 9": "AMD Ryzen 9 8945HS / 7945HX (God Mode Performance)",
    "AMD Ryzen 7": "AMD Ryzen 7 8845HS / 7840HS (Creator/Gamer)",
    "AMD Ryzen 5": "AMD Ryzen 5 7640HS / 5600H (Value Performance)",
    "Intel Core i3": "Intel Core i3-1315U (Basic Work)",
    "Other Intel Processor": "Intel Celeron / Pentium (Budget)",
...
with col_r:
    ...
    with c2:
        cpu_choice = st.selectbox("Processor (All New 2025-26)", list(cpu_display_map.values()))
...
if st.button("Generate AI Price Quote & Recommendations"):
    ...
    # --- CALIBRATION LOGIC (The "Magic" Part) ---
    # Refresh Rate Adjustment
    refresh_val = int(refresh_rate.replace("Hz", "").replace("+", ""))
    hz_bonus = (refresh_val - 60) * 150 # ₹150 per extra Hz
    
    # Purpose Adjustment
    purpose_bonus = 0
    if purpose == "Professional Editing": purpose_bonus = 15000
    elif purpose == "Hardcore Gaming": purpose_bonus = 12000
    elif purpose == "Corporate/Business": purpose_bonus = 5000
    
    # Market Inflation (2026 Adjustment)
    final_price = int((base_pred * 1.12) + hz_bonus + purpose_bonus)

    st.balloons()
    st.markdown(f"<h2 style='text-align: center;'>Estimated 2026 Market Price: ₹{final_price:,}</h2>", unsafe_allow_html=True)
    st.markdown(f"<p style='text-align: center;' class='purpose-tag'>Optimized for: {purpose} @ {refresh_rate}</p>", unsafe_allow_html=True)

    st.markdown("---")
    st.subheader(f"🛒 Top 4 {purpose} Laptops in your range:")
        ...
        with cols[i]:
            st.markdown(f"""
                <div class="card">
                    ...
                    <div class="price-tag">₹{int(row['Price'] * 1.15):,}</div>
                </div>
                """, unsafe_allow_html=True)
